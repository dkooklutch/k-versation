...
export const OPENING_TIMING = { normalMs: 4200, reducedMs: 850, exitMs: 700, rotationSeconds: 15 } as const
...
export default function OpeningSequence() {
  ...
  const close=()=>{try{sessionStorage.setItem('kv-intro-seen','1')}catch{}setVisible(false)}
  return <div className={`opening ${reduced?'is-reduced':''}`} style={{'--opening-exit':`${OPENING_TIMING.exitMs}ms`,'--opening-delay':`${OPENING_TIMING.normalMs-OPENING_TIMING.exitMs}ms`,'--rotation-duration':`${OPENING_TIMING.rotationSeconds}s`} as React.CSSProperties} role="dialog" aria-label="K-VERSATION opening sequence">
    <button className="opening-skip" onClick={close}>Skip</button>
    <div className="korea-stage" aria-hidden="true"><div className="korea-object">
      {Array.from({ length: 10 },(_,i)=><svg key={i} className="korea-layer" style={{'--depth':`${i * 2 - 9}px`} as React.CSSProperties} viewBox="0 0 220 320"><path d={koreaPath}/></svg>)}
    </div></div>
