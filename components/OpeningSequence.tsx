...
export const OPENING_TIMING={durationMs:5600,reducedDurationMs:1400,exitMs:720,rotationSeconds:15}as const
...
const frames=[
 ...
 {src:'/history/05-old-market-1903.jpg',year:'1902—03',caption:'A market · late Joseon'},
 {src:'/history/02-namdaemun-1905.jpg',year:'1905',caption:'Namdaemun · Korean Empire'},
 {src:'/history/06-food-shop-1920.jpg',year:'c. 1920',caption:'A Seoul food shop'},
 {src:'/history/07-independence-1945.jpg',year:'1945',caption:'Independence Day · Seoul'},
 {src:'/history/08-seoul-panorama-1945.jpg',year:'1945',caption:'Seoul from Namsan'},
...
export default function OpeningSequence(){
 ...
 return <div ref={screenRef} className={`opening opening-archive ${reduced?'is-reduced':''} ${entering?'is-entering':''}`} role="status" aria-label={`K-VERSATION opening archive, ${progress} percent loaded`} tabIndex={0} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '||event.key==='Escape'){event.preventDefault();finish()}}}>
  <div className="opening-history" aria-hidden="true">{frames.map((item,index)=>{const distance=Math.min(Math.abs(index-active),frames.length-Math.abs(index-active));return <figure key={item.src} className={index===active?'is-active':distance===1?'is-near':''}><Image src={item.src} alt="" fill sizes="(max-width: 700px) 48vw, 26vw" priority={index<4}/><figcaption><b>{item.year}</b><span>{item.caption}</span></figcaption></figure>})}</div>
  <span className="opening-count">K-V / ARCHIVE 001—008</span>
