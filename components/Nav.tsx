...
export default function Nav() {
  ...
  return <header className="site-header">
    ...
    <Link className="nav-mark" href="/" onClick={e => { if (pathname === '/') { e.preventDefault(); scrollTo({ top: 0, behavior: 'smooth' }) } }} aria-label="K-VERSATION home">K<span>—</span>V</Link>
    <nav className="nav-desktop" aria-label="Primary">
      {links.map(([label, href], i) => <Link key={href} href={href} onClick={()=>setOpen(false)} aria-current={pathname.startsWith(href) ? 'page' : undefined}><small>0{i + 1}</small>{label}</Link>)}
    </nav>
