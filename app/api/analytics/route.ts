...
export async function POST(req:NextRequest){
 ...
 try{
  const b=await req.json(),eventType=cleanText(b.eventType,40),path=cleanText(b.path,300)||'/',pageLoadId=cleanText(b.pageLoadId,100)
  ...
  if((eventType==='page_view'||eventType==='open')&&!/^[a-f0-9-]{36}$/i.test(pageLoadId))return NextResponse.json({error:'A page-load token is required.'},{status:400,headers})
  const row={event_type:eventType,page_path:path,content_id:cleanText(b.contentId,100)||null,content_type:contentType,anonymous_session:cleanText(b.sessionId,100)||crypto.randomUUID(),page_load_id:eventType==='page_view'||eventType==='open'?pageLoadId:null,referrer:cleanText(req.headers.get('referer'),500)||null,country}
  const {error}=await createAdminClient().from('analytics_events').upsert(row,{onConflict:'page_load_id',ignoreDuplicates:true})
