...
export async function getStatistics(): Promise<StatisticsPayload> {
  ...
  const [{data:v,error:vError},{data:a,error:aError},{data:automaticCountries,error:countryError},{data:manualCountries,error:manualCountryError}]=await Promise.all([
    db.from('verified_statistics').select('*').single(),
    db.from('statistic_adjustments').select('statistic_key,adjustment,manual_override'),
    db.from('analytics_events').select('country').not('country','is',null),
    db.from('manual_country_activity').select('country_code,content_views')
  ])
  if(vError) throw vError
  if(aError) throw aError
  if(countryError) throw countryError
  if(manualCountryError) throw manualCountryError
  const source=v as Record<string,unknown>
  verified.site_impressions=Number(source.site_impressions||0)
  verified.content_views=Number(source.content_opens||0)+(manualCountries||[]).reduce((sum,row)=>sum+Number(row.content_views||0),0)
  for(const key of ['conversations','papers','reactions','comments','subscribers'] as StatisticKey[]) verified[key]=Number(source[key]||0)
  verified.countries=new Set([...(automaticCountries||[]).map(row=>String(row.country)),...(manualCountries||[]).filter(row=>Number(row.content_views)>0).map(row=>String(row.country_code))]).size
